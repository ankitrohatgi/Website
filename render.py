...
class Blog:
    ...
    def renderRSS(self, env):
        ...
        for post in self.posts:
            post.getContent(env, "http://arohatgi.info/")

        template = env.get_template(self.feedTemplate)
        pageHtml = template.render(posts=self.posts, base_dir="http://arohatgi.info/")
        rss_file = open(self.outputDir + 'AnkitRohatgi.rss', 'w')
